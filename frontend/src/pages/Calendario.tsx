        ...
        <Dialog open={modalOpen} onOpenChange={setModalOpen}>
          <DialogContent>
            ...
            >
              ...
              {overlapWarning && <div className="text-blue-600 text-sm">{overlapWarning}</div>}
              <div className="flex justify-between gap-2 mt-2">
                {editMode && (
                  <Button type="button" variant="destructive" onClick={handleDelete} className="bg-red-500 text-white hover:bg-red-600">
                    Eliminar
                  </Button>
                )}
                <div className="flex gap-2 ml-auto">
                  <Button type="button" variant="secondary" onClick={() => setModalOpen(false)} className="bg-gray-500 text-white hover:bg-gray-600">
                    Cancelar
                  </Button>
          ...
          <DialogContent>
            ...
            {modalPaciente && (
              <div className="p-4 min-w-[350px] max-w-[500px]">
                ...
                <div className="mb-2 text-sm text-black">{modalPaciente.email} | {modalPaciente.telefono}</div>
                ...
                <ul className="max-h-48 overflow-y-auto text-sm">
                  {modalPaciente.citas?.length === 0 && <li className="text-black">Sin citas registradas</li>}
                  {modalPaciente.citas?.map((cita: any) => (
                    <li key={cita.id} className="mb-1 border-b pb-1">
                      ...
                      <br />
                                              <span className="text-xs text-black">Médico: {cita.usuarios?.nombre} {cita.usuarios?.apellido} | Consultorio: {cita.consultorios?.nombre}</span>
                    </li>
                  ))}
                </ul>
              </div>
  ...
  return (
    <div className="bg-gray-50 rounded-lg p-4 mt-2 flex flex-col gap-2 border">
      ...
      <div className="flex justify-end mt-2">
                          <Button type="button" disabled={isSubmitting} className="h-10 px-6 text-base bg-blue-500 text-white hover:bg-blue-600" onClick={handleSubmit(onSubmit)}>Guardar paciente</Button>
      </div>
    </div>
  );
} 