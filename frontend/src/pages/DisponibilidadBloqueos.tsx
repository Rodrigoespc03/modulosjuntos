      ...
      <div className="w-full max-w-3xl mx-auto">
        ...
        <div className="flex gap-4 mb-6 items-center justify-center">
          ...
          <div className="w-64">
            <Select value={selectedMedico} onValueChange={setSelectedMedico}>
              ...
              <SelectContent className="bg-white text-gray-900">
                {usuarios.length === 0 && <div className="px-4 py-2 text-black">No hay médicos registrados</div>}
                {usuarios.map(u => (
          ...
          <div className="ml-auto flex gap-2">
            <Button variant={tab === 'disponibilidad' ? 'default' : 'outline'} onClick={() => setTab('disponibilidad')} className={tab === 'disponibilidad' ? 'bg-blue-500 text-white hover:bg-blue-600' : 'bg-gray-500 text-white hover:bg-gray-600 border-gray-500'}>Disponibilidad</Button>
            <Button variant={tab === 'bloqueos' ? 'default' : 'outline'} onClick={() => setTab('bloqueos')} className={tab === 'bloqueos' ? 'bg-blue-500 text-white hover:bg-blue-600' : 'bg-gray-500 text-white hover:bg-gray-600 border-gray-500'}>Bloqueos</Button>
          </div>
        </div>
        {usuarios.length === 0 && (
        ...
        {usuarios.length > 0 && tab === 'disponibilidad' && (
          <section>
            <div className="flex justify-between items-center mb-2">
              ...
              <Button onClick={handleNew} className="bg-green-500 text-white hover:bg-green-600">Agregar</Button>
            </div>
              ...
              <table className="w-full border border-gray-300 text-sm bg-white text-gray-900">
                ...
                <tbody>
                  {disponibilidades.length === 0 ? (
                    <tr><td colSpan={4} className="text-center text-black py-4">No hay disponibilidad registrada</td></tr>
                    ...
                    <tr key={d.id} className="hover:bg-gray-50">
                      ...
                      <td>{d.hora_fin}</td>
                      <td>
                        <Button size="sm" onClick={() => handleEdit(d)} className="bg-blue-500 text-white hover:bg-blue-600 mr-2">Editar</Button>
                        <Button size="sm" variant="destructive" onClick={() => removeDisponibilidad(d.id)} className="bg-red-500 text-white hover:bg-red-600">Eliminar</Button>
                      </td>
                    </tr>
                  ))}
                </tbody>
          ...
          <section>
            ...
            {isLoadingBloqueos ? <div className="text-center py-8">Cargando...</div> : errorBloqueos ? <div className="text-red-500">{errorBloqueos}</div> : (
              <table className="w-full border border-gray-300 text-sm bg-white text-gray-900">
                ...
                <tbody>
                  {bloqueos.length === 0 ? (
                    <tr><td colSpan={4} className="text-center text-black py-4">No hay bloqueos registrados</td></tr>
                  ) : bloqueos.map(b => (
                    <tr key={b.id} className="hover:bg-gray-50">
                      ...
                      <td>{b.motivo || '-'}</td>
                      <td>
                        <Button size="sm" onClick={() => handleEdit(b)} className="bg-blue-500 text-white hover:bg-blue-600 mr-2">Editar</Button>
                        <Button size="sm" variant="destructive" onClick={() => removeBloqueo(b.id)} className="bg-red-500 text-white hover:bg-red-600">Eliminar</Button>
                      </td>
                    </tr>
                  ))}
                </tbody>
  ...
  return (
    <form className="space-y-4" onSubmit={e => {
      ...
      <div className="flex gap-2 justify-end">
        <Button type="button" variant="outline" onClick={onCancel} className="bg-gray-500 text-white hover:bg-gray-600 border-gray-500">Cancelar</Button>
        <Button type="submit" disabled={!!error} className="bg-blue-500 text-white hover:bg-blue-600">Guardar</Button>
      </div>
    </form>
  )
}
